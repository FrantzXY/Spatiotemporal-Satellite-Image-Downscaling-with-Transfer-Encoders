...
"""
Semivariogram + ACF/PACF (paper-style)

Modes
-----
1) native     : G5NR (truth) vs MERRA-2 (regridded to G) over full overlap (2005-05-15 .. 2007-06-16).
2) id         : Prediction (downscaled_mean_strict.npy) vs G5NR for dates in pred_days_strict.npy.
3) id-native  : Native MERRA-2 vs native G5NR but only on the ID dates (pred_days_strict.npy).

Semivariogram: γ(h) = 0.5 E[(Z(s) − Z(s+h))²], isotropic, binned by great-circle distance (km).
ACF/PACF: region-mean time series over the Area polygon union (if shapefiles available), else full grid.

Default settings (as requested)
-------------------------------
NDAYS=3, ACF_LAGS=30, MAX_KM=600, NBINS=24, PAIRS=30000, SEED=42

Expected layout for a run directory
-----------------------------------
<rundir>/Season{S}/Area{A}/
  - downscaled_mean_strict.npy
  - pred_days_strict.npy
  - norm_params.npz

CLI examples
------------
# Native: 3 evenly spaced days, with ACF/PACF
python semivariogram_ACF_qc_plot.py native \
  --rundir /path/to/new_large_ddpm_3_with_transfer_result \
  --season 3 --area 0 \
  --outdir /path/to/plots/native_S3A0 \
  --n-days 3 --sample even --max-km 600 --nbins 24 --pairs 30000 \
  --acf-pacf --acf-lags 30 --seed 42 --mask \
  --shapes-root /path/to/shapes \
  --g5nr-nc   /path/to/G5NR_merged_daily_noclip_2005-2007.nc \
  --merra2-nc /path/to/MERRA2_merged_2000-2024_fixed.nc \
  --elev-npy  /path/to/elevation_data.npy

# ID: Prediction vs G5NR on the model’s ID dates
python semivariogram_ACF_qc_plot.py id ... (same common args as above)

# ID-native: native MERRA-2 vs G5NR on the same ID dates
python semivariogram_ACF_qc_plot.py id-native ... (same common args)

Notes
-----
- Use --sync-y pair to force the same Y axis for the two fields plotted for the same day (default).
  Use --sync-y all to force a common Y across all semivariogram figures generated in one run.
"""
