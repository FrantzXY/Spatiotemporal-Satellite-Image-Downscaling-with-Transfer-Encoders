...
# Global normalizaion values for lat and lon
LAT_MIN = 10.875
LAT_MAX = 43.25
LON_MIN = 34.4375
LON_MAX = 80.25
...
class data_processer():
    ...
    def norm_per_array(self, arr: np.ndarray) -> np.ndarray:
        """
        This is a special function to normalize train and test data independently without data leakage.
        It is mathmatically equivaent to nomalize() used below. 
        """
        mn = np.nanmin(arr)
        mx = np.nanmax(arr)
    ...
    def load_data(self, target_variable, file_path_g,
        ...
        '''
        extend: select multiple vars at a time.

        load G5NR, MERRA2, elevation in the outer bound of given shape file
        :param target_variable: name of interested variable to be loaded
        :param file_path_g: a SINGLE file path (rather than 2 paths) to G5NR
        :param file_path_m: file path to MERRA2
        :param file_path_ele: file path to elevation data
        :param file_path_country: list of file paths to country shapefiles.
        :return:
        '''
    ...
    def flatten(self, h_data,
        ...
        """
        Create a proper date range for the G5NR data.
        The G5NR file starts on 2005-05-15.
        WHY: We are replacing this entire block to make the year calculation

        identical to the logic in the small model's AerosolDataset.

        1. The 'days' variable contains 1-based indices from the G5NR period (1 to 763).
        We must first convert these to absolute day indices since 2000-01-01 so that small model actually understands what is going on.
        The G5NR data we use starts on 2005-05-15, which is 1961 days after 2000-01-01.
        """
                    ...
                    if h_data[(t - n_lag + 1):t + 1, (lat - task_lat_dim):lat, (lon - task_lon_dim):lon].shape != (n_lag, task_lat_dim, task_lon_dim):
                        ...
                        if return_nan:
                            ...
                            if return_Y:
                                ...
                                low_slice = l_data[t + 1, lat - task_lat_dim:lat, lon - task_lon_dim:lon][np.newaxis, ...]
                                X_low.append(low_slice)
                                X_ele.append(ele_data[(lat - task_lat_dim):lat, (lon - task_lon_dim):lon])
                                year_norm = year_norm_lookup[t + 1]
                                X_other.append([lat_scaled, lon_scaled, (days[t + 1] % 365) / 365, year_norm])
                        else:
                            window_ok = not np.isnan(h_data[t, lat - 1, lon - 1]) \
                                and not np.isnan(h_data[(t - n_lag + 1):t + 1, (lat - task_lat_dim):lat, (lon - task_lon_dim):lon]).any()
                            if window_ok:
                                X_high.append(
        ...
        if is_perm:
            ...
            if return_Y:
                return np.expand_dims(np.array(X_high, dtype=np.float32), -1)[perm], np.expand_dims(np.array(X_low, dtype=np.float32), -1)[perm], \
                       np.expand_dims(np.array(X_ele, dtype=np.float32), -1)[perm], np.array(X_other, dtype=np.float32)[perm], np.array(Y, dtype=np.float32)[perm]
            else:
                return np.expand_dims(np.array(X_high, dtype=np.float32), -1)[perm], np.expand_dims(np.array(X_low, dtype=np.float32), -1)[perm], \
                       np.expand_dims(np.array(X_ele, dtype=np.float32), -1)[perm], np.array(X_other, dtype=np.float32)[perm]
        if return_Y:
            return np.expand_dims(np.array(X_high, dtype=np.float32), -1), np.expand_dims(np.array(X_low, dtype=np.float32), -1), \
                   np.expand_dims(np.array(X_ele, dtype=np.float32), -1), np.array(X_other, dtype=np.float32), np.array(Y, dtype=np.float32)
        else:
            return np.expand_dims(np.array(X_high, dtype=np.float32), -1), np.expand_dims(np.array(X_low, dtype=np.float32), -1), \
                   np.expand_dims(np.array(X_ele, dtype=np.float32), -1), np.array(X_other, dtype=np.float32)
    ...
    def flatten_transfer(
        ...
        *,
        ...
        """
        Slice ONLY MERRA-2 into overlapping spatio-temporal windows.
        This function is designed to fit special task of tarnsfer model
        of MERRA2 data only. 

        Returns:
        ▸ X_low : (N, n_lag, Htask, Wtask, 1)  float32  
        ▸ Y     : (N, n_pred, Htask, Wtask)    float32   (optional)
        """
